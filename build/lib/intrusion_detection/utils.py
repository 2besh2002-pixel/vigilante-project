...
def generate_pdf_report(report_data: Dict[str, Any], output_path: str):
    ...
    detection_data = [
        ...
        ["Avg False Positive Rate", f"{(detection_summary.get('avg_false_positive_rate') or 0):.2f}%"],
    ]
    # 
    ...
    # All Detections
    story.append(Paragraph("All Detections", styles['Heading2']))
    all_detections = report_data.get('all_detections', [])
    if all_detections:
        det_data = [["ID", "User", "Date", "Total Flows", "Anomalies", "Model ID"]]
        for det in all_detections[:20]:
            det_data.append([
                str(det.get('id', 'N/A')),
                det.get('username', 'N/A'),
                str(det.get('created_at', 'N/A'))[:16],
                f"{det.get('total_flows', 0):,}",
                str(det.get('anomalies_detected', 0)),
                str(det.get('model_id', 'N/A')),
            ])
        det_table = Table(det_data, colWidths=[0.5*inch, 1*inch, 1.5*inch, 1.2*inch, 1*inch, 1*inch])
        ...
        story.append(Paragraph("No detections found in the period.", styles['Normal']))
    story.append(Spacer(1, 20))

    # All Models
    story.append(Paragraph("All Models in System", styles['Heading2']))
    ...
    if all_models:
        ...
        for model in all_models[:20]:
            ...
            model_data.append([
                ...
                str(model.get('created_at', 'N/A'))[:10],
            ])
        models_table = Table(model_data, colWidths=[0.4*inch, 1.8*inch, 0.8*inch, 1.2*inch, 0.8*inch, 0.8*inch, 0.8*inch])
